...
export const RichTextBlok: FC<RichTextBlokProps> = ({ blok }) => {
  ...
  return (
    <div
      ...
      className="prose text-pretty richtext-external-link-indicator [&_h1]:text-balance [&_h2]:text-balance [&_h3]:text-balance [&_h4]:text-balance [&_:not(pre)>code]:rounded [&_:not(pre)>code]:bg-black [&_:not(pre)>code]:px-1.5 [&_:not(pre)>code]:py-0.5 [&_:not(pre)>code]:font-bold [&_:not(pre)>code]:text-yellow-400 [&_:not(pre)>code]:whitespace-nowrap [&_:not(pre)>code]:before:content-none [&_:not(pre)>code]:after:content-none"
    >
